...
export default function WeekCalendar({ weekStart, selectedDateISO, taskCounts, onSelectDate, onPrevWeek, onNextWeek, onToday, onJumpToDate, t, lang }) {
    ...
    <div className="calendar-panel mb-3">
      <div className="d-flex justify-content-between align-items-center mb-2 flex-wrap gap-2">
        <div className="d-flex align-items-center gap-2">
          <button className="btn-ghost-icon btn-ghost-icon-sm" onClick={onPrevWeek} aria-label={t('prevWeek')}>
            <i className="bi bi-chevron-left" />
          </button>
          <span className="calendar-month text-capitalize">{formatMonthYear(weekStart, lang)}</span>
          <button className="btn-ghost-icon btn-ghost-icon-sm" onClick={onNextWeek} aria-label={t('nextWeek')}>
            <i className="bi bi-chevron-right" />
          </button>
        </div>
        <div className="d-flex gap-2">
          <label className="btn btn-pill btn-outline-soft btn-sm calendar-jump-btn mb-0">
            <i className="bi bi-calendar3 me-1" />{t('pickDate')}
            <input
              type="date"
      ...
      <div className="calendar-grid">
        {days.map(day => {
          ...
          return (
            ...
            >
              <span className="calendar-weekday">{weekdayShort(day, lang)}</span>
